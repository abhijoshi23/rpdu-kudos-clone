...
export default function RecipeReviewCard({ id, kudos, kudoShare, user }) {
    ...
    const handleLikedClick = () => {
        clickType = liked ? 'dislike' : 'like'
        setLiked(!liked)

        fetch(`api/kudos/like/${id}`,
            {
                method: "PATCH",
                headers: {
    ...
    return (
        ...
        <Card sx={{ maxWidth: 345, boxShadow: 10 }} className="d-flex flex-column" >
            ...
            <CardActions disableSpacing className='mt-auto' sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
                ...
                <IconButton aria-label="share" onClick={handleShareClicked}>
                    <ShareIcon />
                </IconButton>
            </CardActions>
        </Card>
